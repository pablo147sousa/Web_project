...
def new_author_view(request):
    ...
    context = {'form': form}
    return render(request, 'novo_autor.html', context)
...
def edita_autor_view(request, autor_id):
    ...
    return render(request, 'edita_autor.html', context)
...
def novo_artigo_view(request, autor_id):
    ...
    context = {'form': form}
    return render(request, 'novo_artigo.html', context)
...
def edit_article_view(request, article_id):
    ...
    if request.POST:
        ...
        if form.is_valid():
            form.save()
            return redirect('artigos:article',article.id)
    else:
        form = ArtigoForm(instance=article)  # cria formulário com dados da instância artigo

    context = {'form': form, 'article':article}
    return render(request, 'edita_artigo.html', context)
